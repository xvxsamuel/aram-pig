...
export default function SearchBar({ className = 'w-full max-w-3xl', ddragonVersion: propVersion }: Props) {
  ...
  const displayedSummoners = useMemo((): (RecentSummoner | SearchHistoryItem)[] => {
    ...
    return searchHistory
      .filter(h => h.type === 'summoner')
      .slice(0, 5) as SearchHistoryItem[]
  ...
  const displayedChampionHistory = useMemo((): SearchHistoryItem[] => {
    ...
    return searchHistory
      .filter(h => h.type === 'champion')
      .slice(0, 3) as SearchHistoryItem[]
  }, [name, searchHistory])
  ...
  const allItems = useMemo(() => {
    ...
    return items
  }, [displayedChampions, displayedChampionHistory, displayedSummoners])
          ...
          {showDropdown && (
            <>
              ...
              >
                ...
                {/* Summoners Section - show when searching, has results, or showing history */}
                {(displayedSummoners.length > 0 || displayedChampionHistory.length > 0 || !name.trim() || (name.trim() && isSearchingSummoners)) && (
                  <div>
                    ...
                    {/* Champion history items (when not searching) */}
                    {displayedChampionHistory.map((champ, idx) => {
                      const itemIdx = getItemIndex('champion-history', idx)
                      return (
                        <button
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-subtitle">Champion</div>
                          </div>
                          {selectedIndex === itemIdx && (
                            <span className="ml-auto text-xs text-text-muted">↵</span>
                          )}
                    ...
                    {/* Summoner items */}
                    {displayedSummoners.map((summoner, idx) => {
                      const itemIdx = getItemIndex('summoner', idx)
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-subtitle flex items-center gap-1.5">
                              ...
                              {s.summoner_level && s.summoner_level > 0 && (
                                <span>Level {s.summoner_level}</span>
                              )}
                            </div>
                          </div>
                          {selectedIndex === itemIdx && (
