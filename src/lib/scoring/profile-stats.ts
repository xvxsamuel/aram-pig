...
export async function recalculateProfileChampionStats(puuid: string): Promise<void> {
  ...
  for (const [champName, stats] of Object.entries(championStats)) {
    ...
    champions[champName] = {
      ...
      avgPigScore: stats.pigScoreCount > 0 
        ? Math.round((stats.pigScoreSum / stats.pigScoreCount) * 10) / 10 
        : null
    }
  }
  ...
  if (updateError) {
    ...
    console.log(`[UpdateProfile] Cached profile_data for ${puuid}: ${Object.keys(champions).length} champions, ${matchCount} total games`)
  }
}
...
export async function getTrackedPlayersFromMatches(participantPuuids: string[]): Promise<string[]> {
  ...
  const { data: trackedSummoners, error } = await supabase
    .from('summoners')
    .select('puuid')
