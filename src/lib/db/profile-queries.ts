...
function transformToMatchDataParticipant(
  ...
  const gameName =
    nameOverride && p.puuid === nameOverride.puuid ? nameOverride.gameName : (p.riot_id_game_name as string) || ''
  const tagLine =
    nameOverride && p.puuid === nameOverride.puuid ? nameOverride.tagLine : (p.riot_id_tagline as string) || ''
...
export async function getProfileIcons(puuids: string[]): Promise<Record<string, number>> {
  ...
  const { data } = await supabase.from('summoners').select('puuid, profile_icon_id').in('puuid', puuids)
