...
import { getKillDeathSummary } from '../game/kill-timeline'
 ...
 */
export async function autoEnrichRecentMatches(matchIds: string[], region: PlatformCode): Promise<number> {
  ...
  for (const match of matchesToEnrich) {
    try {
      ...
      await supabase.from('matches').update({ timeline_data: timeline }).eq('match_id', match.match_id)
      ...
      for (const participant of participants) {
        ...
        // Calculate pig score (skip remakes)
        let pigScore: number | null = null
        if (!matchParticipant.gameEndedInEarlySurrender) {
          const breakdown = await calculatePigScoreWithBreakdown({
            championName: participant.champion_name,
            damage_dealt_to_champions:
              participant.match_data?.stats?.damage || matchParticipant.totalDamageDealtToChampions || 0,
            total_damage_dealt:
              participant.match_data?.stats?.totalDamageDealt || matchParticipant.totalDamageDealt || 0,
            total_heals_on_teammates:
              participant.match_data?.stats?.totalHealsOnTeammates || matchParticipant.totalHealsOnTeammates || 0,
...
export async function getStoredTimeline(matchId: string) {
  ...
  const { data } = await supabase.from('matches').select('timeline_data').eq('match_id', matchId).single()
