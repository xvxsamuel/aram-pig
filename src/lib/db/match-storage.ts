  ...
  try {
    ...
    const participantRows = matchData.info.participants.map((p, index) => {
      ...
      const timelineBuildOrder = itemPurchases
        .filter(
          purchase => purchase.action === 'buy' && isFinishedItem(purchase.itemId) && finalItemSet.has(purchase.itemId)
        )
        .slice(0, 6)
        .map(p => p.itemId)
    ...
    if (patchAccepted) {
      ...
      console.log(
        `[STATS] Stored ${matchData.metadata.matchId} (+${statsData.length} participants, buffer: ${getStatsBufferCount()}, ${getAggregatedChampionCount()} champions)`
      )
    } else if (isRemake) {
