...
async function cleanupStaleJobs(supabase: any) {
  ...
  await supabase.from('update_jobs').delete().in('status', ['completed', 'failed']).lt('completed_at', twoMinutesAgo)
}
...
export async function POST(request: Request) {
  try {
    ...
    const recentlyCompleted = job.completed_at && new Date(job.completed_at).getTime() > Date.now() - 5 * 60 * 1000
    ...
    // calculate progress
    const progressPercentage = job.total_matches > 0 ? Math.round((job.fetched_matches / job.total_matches) * 100) : 0
    ...
    return NextResponse.json({ error: error.message || 'failed to get update status' }, { status: 500 })
  }
}
