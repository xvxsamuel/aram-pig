...
export async function POST(request: Request) {
  try {
    ...
    const { data: existingMatches } = await supabase.from('summoner_matches').select('match_id').eq('puuid', puuid)
    ...
    return NextResponse.json({ error: error.message || 'Failed to get match count' }, { status: 500 })
  }
}
